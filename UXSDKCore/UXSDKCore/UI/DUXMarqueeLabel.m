...
NSString *const kDUXMarqueeLabelControllerRestartNotification = @"DUXMarqueeLabelViewControllerRestart";
NSString *const kDUXMarqueeLabelShouldLabelizeNotification = @"DUXMarqueeLabelShouldLabelizeNotification";
NSString *const kDUXMarqueeLabelShouldAnimateNotification = @"DUXMarqueeLabelShouldAnimateNotification";
NSString *const kDUXMarqueeLabelAnimationCompletionBlock = @"DUXMarqueeLabelAnimationCompletionBlock";
...
@implementation DUXMarqueeLabel

#pragma mark - Class Methods and handlers

+ (void)restartLabelsOfController:(UIViewController *)controller {
    [DUXMarqueeLabel notifyController:controller
                       withMessage:kDUXMarqueeLabelControllerRestartNotification];
}

+ (void)controllerViewWillAppear:(UIViewController *)controller {
    [DUXMarqueeLabel restartLabelsOfController:controller];
}
...
+ (void)controllerLabelsShouldLabelize:(UIViewController *)controller {
    [DUXMarqueeLabel notifyController:controller
                       withMessage:kDUXMarqueeLabelShouldLabelizeNotification];
}

+ (void)controllerLabelsShouldAnimate:(UIViewController *)controller {
    [DUXMarqueeLabel notifyController:controller
                       withMessage:kDUXMarqueeLabelShouldAnimateNotification];
}
...
- (void)setupLabel {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewControllerShouldRestart:) name:kDUXMarqueeLabelControllerRestartNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldLabelize:) name:kDUXMarqueeLabelShouldLabelizeNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldAnimate:) name:kDUXMarqueeLabelShouldAnimateNotification object:nil];
...
#pragma mark - DUXMarqueeLabel Heavy Lifting
...
- (void)scrollAwayWithInterval:(NSTimeInterval)interval delayAmount:(NSTimeInterval)delayAmount shouldReturn:(BOOL)shouldReturn {
    ...
    [awayAnim setValue:@(YES) forKey:kDUXMarqueeLabelAnimationCompletionBlock];
...
- (void)scrollContinuousWithInterval:(NSTimeInterval)interval
    ...
    [labelAnimation setValue:@(YES) forKey:kDUXMarqueeLabelAnimationCompletionBlock];
...
- (void)setNumberOfLines:(NSInteger)numberOfLines {
    // By the nature of DUXMarqueeLabel, this is 1
    [super setNumberOfLines:1];
}

- (void)setAdjustsFontSizeToFitWidth:(BOOL)adjustsFontSizeToFitWidth {
    // By the nature of DUXMarqueeLabel, this is NO
...
@implementation UIView (DUXMarqueeLabelHelpers)
// Thanks to Phil M
...
@implementation CAMediaTimingFunction (DUXMarqueeLabelHelpers)
...
- (CGFloat)solveTForY:(CGFloat)y_0 withEpsilon:(CGFloat)epsilon controlPoints:(NSArray *)controlPoints
{
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (fabs(df0) < 1e-6) {
            NSLog(@"DUXMarqueeLabel: Newton's Method failure, small/zero derivative!");
            break;
        }
        // Else recalculate t1
        t1 = t0 - f0/df0;
    ...
    NSLog(@"DUXMarqueeLabel: Failed to find t for Y input!");
    return t0;
}
